...
$(document).ready(function() {
	$('#statementTable').datagrid({
		title : '收支明细',
		loadMsg : '数据装载中......',
		//url:'${ctx}/financeStatement/load.do',
		fit:true,
		fitColumns:false,
		nowrap : true,
		...
		toolbar :[
		    {
				...
				iconCls : 'icon-excel',
				handler : function() {
					$("#statementTable").datagrid("loading");
					var rows = $("#statementTable").datagrid('getSelections');
					var params = $("#searchStatementForm").serialize();
					$.ajax({
						type : "POST",
						dataType:"json",
						url : "${ctx}/financeStatement/exportExeclCus.do",
						data : getOrderId(rows)+ "&" +params,
						success : function(response) {
							if(response.success){
							window.location.href ="${ctx}/excel/"+response.url;
							$("#statementTable").datagrid("loaded");
						}else{
		...
		columns : [[
		        ...
		        {title : '创建时间',field : 'createTimeString',rowspan : 1,align : 'center',width : '220'},
		        ...
		        {title : '客户名称',field : 'fname',rowspan : 1,align : 'center',width : '200'},
	...
	$('#statementTable').datagrid('getPager').pagination({
	...
	//window.toolbarSearch("searchStatementForm","fuserId","fusername","searchButton",1);
	//客户下拉框数据加载,在datagrid加载完成以后配置
	...
	$("#userid").combobox({
		...
		onSelect:function(record){
			$.ajax({
				type : "POST",
				dataType:"json",
				url : "${ctx}/statement/loadHuoBill.do?userroleid="+record.optionId,
				success : function(response) {
					$("#account").empty();
					...
					'<td class="m-info-title" style="padding-left:35px">充值余额:'+response.fcbalance+'</td>'+
					'<td class="m-info-title" style="padding-left:35px">充值送余额:'+response.fgift+'</td>'+
					'<td class="m-info-title" style="padding-left:35px">支出运费余额:'+response.fcostFreight+'</td>'+
					'<td class="m-info-title" style="padding-left:35px">优惠券抵扣金额:'+response.fgoodRoll+'</td>'+
					'<td class="m-info-title" style="padding-left:35px">异常收入金额:'+response.freceive+'</td>'+
...
// 		alert(toJOSNStr($("#searchStatementForm").serializeArray()));
       var opts = $("#statementTable").datagrid("options");
	    opts.url = '${ctx}/financeStatement/load.do';
		$('#statementTable').datagrid('load', JSON.parse(toJOSNStr($("#searchStatementForm").serializeArray())));
	});
});
			...
			<form id="searchStatementForm" method="post">
				...
				<table style="margin: 0 auto; margin-left:200px;  margin-top: 20px;">
					<tr>
						...
						<td>
						<select id="userid" name="fuserid" panelHeight="200">
