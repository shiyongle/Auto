...
public class DongjingClient {
	...
	private void GetURL(String Method) {
		...
		try {
			...
			if (content.equals("")) {
				getUrl = new URL(Url + Method + ".net");// "GetUserList.do"
			} else {
				getUrl = new URL(Url + Method + ".net?" + content);// "GetUserList.do"
			}
	...
	private void PostURL(String Method) {
		...
		try {
			...
			URL serverUrl = new URL(Url + Method + ".net"); // "logonAction.do"
