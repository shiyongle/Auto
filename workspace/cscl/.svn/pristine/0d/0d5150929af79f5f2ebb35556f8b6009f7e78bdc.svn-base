...
$(document).ready(function() {
	...
	$('#complainTable').datagrid({
		...
		toolbar : [{
							...
							iconCls : 'icon-add',
							handler : function() {
								$("#complainOrder_Window").window({
									title : "异常创建",
									href : '${ctx}/usercomplain/add.do',
									width : 900,
									top:0,
									draggable:true,
									resizable:false,
							...
							handler : function() {
								...
								if(rows.length<=0){
									...
									$.messager.alert('提示', '每次只能选择一条记录！', 'info');
								    	 ...
								    	 $("#complainOrder_Window").window({
											title : "异常查看",
											href : '${ctx}/usercomplain/view.do?id='+rows[0].fid,
											width : 900,
											top:0,
											draggable:true,
											resizable:false,
							...
							handler : function() {
								...
								if(rows.length<=0){
									...
									$.messager.alert('提示', '每次只能选择一条记录！', 'info');
								    	 ...
								    	 $("#complainOrder_Window").window({
											title : "编辑异常",
											href : '${ctx}/usercomplain/edit.do?id='+rows[0].fid,
											width : 900,
											top:0,
											draggable:true,
											resizable:false,
						...
						handler : function() {
							...
							if(rows.length<=0){
						    	 $.messager.alert('提示', '请选择记录！', 'info');
						       	 ...
						       	 if(rows[0].fisdeal==0 || rows[0].fisdeal==2){
						    		 $.messager.alert('提示','请选择已审核的记录！');
						    	 }
						       	 else{						       		
							    	 $("#appendcomplainOrder_Window").window({
										title : "追加异常",
										href : '${ctx}/usercomplain/appendcomplain.do?fid='+rows[0].fid+'&orderId='+rows[0].forderId,
										width: 550,
										height:320,
										draggable:true,
										resizable:false,
						...
						handler : function() {
							var rows = $("#complainTable").datagrid('getSelections');
							 if(rows.length<=0){
							 	...
							 	$.ajax({
							 			type : "POST",
							 			url : '${ctx}/usercomplain/del.do?'+toStrId(rows),
...
// 									data : params,
		...
		columns :[
		           [
					...
					{title : '创建时间',field : 'fcreateTimeString',rowspan : 2,align : 'center',width : '150'}
		           ],
		           [
		            /*司机处理*/
        ...
        onSelect:function(rowIndex, rowData){
        	var selections = $("#complainTable").datagrid('getSelections');
    		dealJust(selections);/***受理状态判断*/
           },
...
function applyToType(value,rowData,rowIndex){
	...
	function clock(value,rowData,rowIndex){
		if(value==null){
			return "- -";
		}else{
...
//受理状态判断
function dealJust(selections){
	for(var i = 0;i<selections.length;i++){
		   for(var j=0;j<selections.length;j++){
			  if( selections[j].fisdeal!=selections[i].fisdeal){				  		
					return
				 }
		     }
			if((selections[i].fisdeal== 0)){
				$("#delComplainOrder,#editComplainOrder").linkbutton('enable');
			}else{
				$("#delComplainOrder,#editComplainOrder").linkbutton('disable');
				return
			}
		}	
}
...
<body>
	<div class="easyui-layout" data-options="fit:true" style="border-left: 0px; border-right: 0px;">
			...
			<div id="complainOrder_Window" style="margin: 0 auto; overflow:hidden;"></div>
			<div id="appendcomplainOrder_Window" style="margin: 0 auto; overflow:hidden;"></div>
	</div>
</body>
</html>