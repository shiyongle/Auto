...
$(document).ready(function() {
	...
	$("#searchButton").click(function() {
		$('#freightSettingTable').datagrid('load', JSON.parse(toJOSNStr($("#searchForm").serializeArray())));
	});
	$('#freightSettingTable').datagrid({
		title : '协议司机计价规则',
		loadMsg : '数据装载中......',
		...
		toolbar : [{
					...
					handler : function() {
							$("#createWindow").window({
								title : "新建",
								href : '${ctx}/protocol/addDriverRule.do',
								width : 300,
								height :410,
								draggable:true,
								resizable:false,
							...
							iconCls : "icon-edit",
							handler : function() {
								var rows = $("#freightSettingTable").datagrid('getSelections');
								if(rows.length<=0){
							    	 ...
							    	 $("#createWindow").window({
										title : "修改",
										href : '${ctx}/protocol/editDriverRule.do?id='+rows[0].id,
										width : 300,
										height :410,
										draggable:true,
										resizable:false,
		...
		columns : [[
					...
					{title : '保底公里',field : 'startKilometre',rowspan : 1,align : 'center',width : '200'},
		            {title : '保底价',field : 'startPrice',rowspan : 1,align : 'center',width : '200'},
		            {title : '超过公里/单价',field : 'outKilometre',rowspan : 1,align : 'center',width : '200'},
		            {title : '超出点数单价(元)',field : 'foutopint',rowspan : 1,align : 'center',width : '200'},
		            {title : '装货费',field : 'fadd_service_1',rowspan : 1,align : 'center',width : '200'},	
		            {title : '卸货费',field : 'fadd_service_2',rowspan : 1,align : 'center',width : '200'},		 
	...
	$('#freightSettingTable').datagrid('getPager').pagination({
...
function ableEditButton(){
	var selections = $("#freightSettingTable").datagrid('getSelections');
			...
			<div data-options="region:'north',title:'查询条件'" style="height: 100px; border: 0px;">
				<form action="" id="searchForm">
					<table style="margin: 0 auto; margin-top: 20px;">
						<td>
							<span class="m-info-title">客户名：</span>
