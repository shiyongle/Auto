...
//		}
		...
		user.put("fpassword", MD5Util.getMD5String(fpassword));
		...
		if(jo.getBoolean("success")){
			...
			if(csclUser==null){		
				...
				csclUser.setVmiUserName("null".equals(returnObject.get("faccount").toString())?returnObject.get("fphone").toString():returnObject.get("faccount").toString());
	...
	public String logon(HttpServletRequest request, HttpServletResponse response)
		...
		if(urlist.size()>0){
			...
			jsonObject.put("password", MD5Util.getMD5String(fpassword));
			...
			if (jo.getBoolean("success")) {
				...
				String feffect = "0";
				feffect = returnObject.get("feffect").toString();//是否启用、禁用	0禁用1启用
				if(feffect.equals("0")){
					map.put("success", "false");
...
//					user.setFpaypassword(pw);
	...
	public String setPassword(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		jsonObject.put("oldPassword", MD5Util.getMD5String(oldpassword));
		jsonObject.put("newPassword", MD5Util.getMD5String(newpassword));
