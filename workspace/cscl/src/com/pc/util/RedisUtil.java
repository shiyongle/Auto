...
public class RedisUtil {
	...
	public static JedisPool getJedisPool() {
		if (pool == null) {
			Properties properties = RedisUtil.getProperties();
			...
			config.setMaxWaitMillis(1000 * maxWaitMillis);
	...
	public static void Lock(String LockName) throws InterruptedException{
		while (true) {
			...
			if (RedisUtil.setnx(LockName, new Date().toString()) != 0) {
				//设置锁的时间为10秒
				RedisUtil.pexpire(LockName, 9000000L);
				break;
			}
		}
	}
	
	public static void LockMethod(String LockName) throws InterruptedException{
		while (true) {
			Thread.sleep(5);
			//生成锁  如果能set进去返回值为0
			if (RedisUtil.setnx(LockName, new Date().toString()) != 0) {
	...
	public static HashMap<String, Object> IsSystemDefend(Integer ftype,Integer urId) throws InterruptedException{
		Jedis jedis = null;
		boolean IsNotAllowPerson = true;
		HashMap<String, Object> map = null;
		try {
			jedis = getJedisPool().getResource();
			if(ftype == 0 ){
				if(jedis.get("NoOrderStartTime")!=null && !"".equals(jedis.get("NoOrderStartTime")) && jedis.get("NoOrderEndTime")!=null && !"".equals(jedis.get("NoOrderEndTime")) ){
					SimpleDateFormat formatTemp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					Date now = new Date();// 当前时间
					String[] urids = null;
					if (RedisUtil.get("NoOrderAllowPerson")!=null && !"".equals(jedis.get("NoOrderAllowPerson"))){
						urids = RedisUtil.get("NoOrderAllowPerson").split(",");
					}
					for(int i=0;i<urids.length;i++){
						if(Integer.valueOf(urids[i]).intValue() == urId.intValue()){
							IsNotAllowPerson = false;
						}
					}
					
					if(IsNotAllowPerson){
						if (formatTemp.parse(RedisUtil.get("NoOrderStartTime")).before(now) && formatTemp.parse(RedisUtil.get("NoOrderEndTime")).after(now)) {
							map = new HashMap<String, Object>();
							map.put("success", "false");
							map.put("msg", RedisUtil.get("NoOrderStartTime") + "至" + RedisUtil.get("NoOrderEndTime") + RedisUtil.get("NoOrderContentkey"));
							return map;
						}
					}
					/*if (!(RedisUtil.get("NoOrderallowPerson").contains(urId+""))) {
						if (formatTemp.parse(RedisUtil.get("NoOrderStartTime")).before(now) && formatTemp.parse(RedisUtil.get("NoOrderEndTime")).after(now)) {
							map.put("success", "false");
							map.put("msg", RedisUtil.get("NoOrderStartTime") + "至" + RedisUtil.get("NoOrderEndTime") + RedisUtil.get("NoOrdercontentkey"));
							return map;
				...
				if(jedis.get("OnlineTestStartTime")!=null && !"".equals(jedis.get("OnlineTestStartTime")) && jedis.get("OnlineTestEndTime")!=null  && !"".equals(jedis.get("OnlineTestEndTime")) ){
					SimpleDateFormat formatTemp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					Date now = new Date();// 当前时间
					String[] urids = null;
					if (RedisUtil.get("OnlineTestAllowPerson")!=null && !"".equals(jedis.get("OnlineTestAllowPerson"))){
						urids = RedisUtil.get("OnlineTestAllowPerson").split(",");
					}
					...
					if(IsNotAllowPerson){
						if (formatTemp.parse(RedisUtil.get("OnlineTestStartTime")).before(now) && formatTemp.parse(RedisUtil.get("OnlineTestEndTime")).after(now)) {
							map = new HashMap<String, Object>();
							map.put("success", "false");
							map.put("msg", RedisUtil.get("OnlineTestStartTime") + "至" + RedisUtil.get("OnlineTestEndTime") + RedisUtil.get("OnlineTestContentkey"));
							return map;
						}
					}
					/*if (!(RedisUtil.get("OnlineTestAllowPerson").contains(urId+""))) {
						if (formatTemp.parse(RedisUtil.get("OnlineTestStartTime")).before(now) && formatTemp.parse(RedisUtil.get("OnlineTestEndTime")).after(now)) {
							map.put("success", "false");
							map.put("msg", RedisUtil.get("OnlineTestStartTime") + "至" + RedisUtil.get("OnlineTestEndTime") + RedisUtil.get("OnlineTestContentkey"));
							return map;
