...
public class UserRoleController extends BaseController {
	...
	public String checkuser(HttpServletRequest request, HttpServletResponse reponse) throws Exception {
		...
		NewDongjingClient callback = ServerContext.createInnerUserClient();
		callback.setMethod("djauth/getcode");
		callback.setRequestProperty("username", fname);
		callback.SubmitData();
		JSONObject callbackObject = net.sf.json.JSONObject.fromObject(callback.getResponse().getResultString());
		if (!callbackObject.getBoolean("success")) {
			return writeAjaxResponse(reponse, JSONUtil.getJson("failure"));
		}
		JSONObject  callbackJsonObject =JSONObject.fromObject(callbackObject.get("data"));
		String code=callbackJsonObject.get("code").toString();
		
		
		NewDongjingClient a = ServerContext.createInnerUserClient();
		a.setMethod("djauth/applogin");
		a.setRequestProperty("username", fname);
		a.setRequestProperty("password", MD5Util.getMD5String(fpassword+code));
		a.setRequestProperty("appname", "DJCRM");
		a.SubmitData();
		JSONObject jbsc = net.sf.json.JSONObject.fromObject(a.getResponse().getResultString());
		if (!jbsc.getBoolean("success")) {
			return writeAjaxResponse(reponse, JSONUtil.getJson("failure"));
		} else {
			HashMap<String, Object> m = new HashMap<String, Object>();
			
			NewDongjingClient client=ServerContext.createInnerUserClient();
			client.setMethod("djauth/getUserByToken");
			Object tokenObject=JSONObject.fromObject(jbsc.get("data")).get("token");
			if (tokenObject==null) {
				return writeAjaxResponse(reponse, JSONUtil.getJson("failure"));
			}
			client.setRequestProperty("token", tokenObject.toString());
			client.SubmitData();
			String resultString = client.getResponse().getResultString();
			if (StringUtils.isEmpty(resultString)||resultString.contains("false")) {
				return writeAjaxResponse(reponse, JSONUtil.getJson("failure"));
			}
			JSONObject returnUser = JSONObject.fromObject(JSONObject.fromObject(JSONObject.fromObject(resultString).get("data")).get("user"));
			String fid = returnUser.get("id").toString();
			String fvmiName = returnUser.get("uname").toString();
			String fvmiPhone = returnUser.get("uphone").toString();
			CL_UserRole userRole = null;
			userRole = userRoleDao.getByVmiUserFidAndRoleId(fid, 3);
			if (userRole == null) {
