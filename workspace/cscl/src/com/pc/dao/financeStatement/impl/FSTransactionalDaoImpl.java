...
@Service("fsTransactionalDao")
public class FSTransactionalDaoImpl extends BaseDao<CL_FinanceStatement, java.lang.Integer>
	...
	@Transactional
	public int updateBusinessTypeTrans(OrderMsg ordermsg, HashMap<String, Object> param) {
		CL_FinanceStatement statement = null;
		int resultStatus = 0;
		List<CL_FinanceStatement> statementlist = financeStatementDao.getByNumber(ordermsg.getOrder());
		...
		if (ordermsg.getUserId() != null && ordermsg.getUserId() == -1) {
			if (statement.getFstatus() == 1) {
				...
				financeStatementDao.update(statement);
...
//			this.orderDao.update(order);
	 ...
	 *            出错提示内容
	 */
	public void writePayErrorLog(CL_FinanceStatement statement, int level, String context) {
		CL_PayLogs payLogs = new CL_PayLogs();
		payLogs.setFcreationTime(new Date());
		if(statement == null || "".equals(statement)){
			payLogs.setFcreator("administrator");
			payLogs.setFcreatorID(1301);
			payLogs.setFpayData("支付系统传输错误订单号！");
			payLogs.setFfinanceNum("支付系统传输错误订单号！");
		}else{
...
//						this.update(statement);
					...
					if (statement.getFbusinessType() == 12
						...
						if (financeStatementDao.findFrelatedIdList(
...
//							toParm.put("FinanceDao", this);
							toParm.put("FinanceDao", financeStatementDao);
...
//									this.update(statement);
		...
		if (statement.getFbusinessType() == 6) {// 充值送优惠券
			...
			BigDecimal MinDecimal = BigDecimal.ZERO;
			BigDecimal subDecimal = BigDecimal.ZERO;// 金额减去最大区间剩余的钱
			int activityCount = 0;
			if (acticityList.size() > 0) {
				MinDecimal = acticityList.get(0).getFtopUpDollars();// 先随便定义一个最小的区间
