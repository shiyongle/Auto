...
function listener(selectivity, $input) {
    ...
    function keyReleased(event) {
        ...
        if (keydownCanceled) {
            ...
            } else if (selectivity.options.showDropdown !== false && selectivity.enabled) {
                open();
            }
...
Selectivity.Locale = {
    ...
    noResults: '未找到结果',
    noResultsForTerm: function(term) { return '未找到结果: <b>' + escape(term) + '</b>'; }
...
Selectivity.Templates = {
    ...
    selectCompliance: function(options) {
        ...
        return ('<select '+((typeof options.selectId == 'string')? 'id="'+options.selectId+'"': '')+' name="' + options.name + '"' + (options.mode === 'multiple' ? ' multiple' : '') + '></select>');
    },
...
function bindTraditionalSelectEvents(selectivity) {
    ...
    $el.on('selectivity-init', function(event, mode) {
        $el.append(selectivity.template('selectCompliance', {
            ...
            selectId: selectivity.options.selectId
