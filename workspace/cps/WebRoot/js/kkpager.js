...
KKPager.prototype = {
		...
		generPageHtml : function(config,enforceInit){
			//alert(enforceInit+"--------"+this.inited+"--------"+!this.inited+"--------"+(enforceInit || !this.inited))
			//console.log(config);
			...
			var gopage_info = '';
			if(this.isGoPage){
				var obj = window[this.pagerid]? this.pagerid: 'kkpager';
				gopage_info = '&nbsp;'+this.lang.gopageBeforeText+'<span id="'+this.gopageWrapId+'">'+
					'<input type="button" id="'+this.gopageButtonId+'" onclick="'+obj+'.gopage()" value="'+this.lang.gopageButtonOkText+'" />'+
					'<input type="text" id="'+this.gopageTextboxId+'" onfocus="'+obj+'.focus_gopage()"  onkeypress="if((KeyCode>=48)&&(KeyCode<=57)||(KeyCode>=96)&& (KeyCode<=105)||(KeyCode>=37)&&(KeyCode<=40)||KeyCode===8||KeyCode==46){ }else{if (window.event){event.returnValue = false;}else{evt.preventDefault();}}"   onblur="'+obj+'.blur_gopage()" value="'+this.pno+'" /></span>'+this.lang.gopageAfterText;
			}
			...
			if(config.total <= 8){
				...
				if(config.pno <= 5){
					...
					var begin = config.pno - 2;
					var end = config.pno + 2;
					if(end > config.total){
						end = config.total;
						begin = end - 4;
		...
		init : function(config){
			...
			if(config.pagerid){
				var id = this.pagerid = config.pagerid;
				window[id]=this;
			}
			...
			//alert("当前页为:"+this.pno);
			//alert("总页码为:"+this.total);
			this.inited = true;
		},
		_getHandlerStr : function(n){
			if(this.mode == 'click'){
				var obj = window[this.pagerid]? this.pagerid: 'kkpager';
				return 'href="'+this.getHref(n)+'" onclick="return '+obj+'._clickHandler('+n+')"';
			}
			//link模式，也是默认的
