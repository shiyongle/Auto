/*
 * jQWidgets v3.9.1 (2015-Oct) Copyright (c) 2011-2015 jQWidgets. License:
...
(function(i) {
		...
		I.toJSON = function(X, O) {
			...
			function M(Z) {
				return '"' + Z.replace(T, function(aa) {
							var ab = W[aa];
							return typeof ab === "string" ? ab : "\\u"
									+ ("0000" + aa.charCodeAt(0).toString(16))
											.slice(-4)
						}) + '"'
			}
			function N(Z) {
			...
			function P(aa) {
				...
				if (isFinite(aa.valueOf())) {
					Z = aa.getUTCFullYear() + "-" + N(aa.getUTCMonth() + 1)
							+ "-" + N(aa.getUTCDate()) + "T"
							+ N(aa.getUTCHours()) + ":" + N(aa.getUTCMinutes())
							+ ":" + N(aa.getUTCSeconds()) + 'Z"'
				} else {
					Z = "null"
				}
				return Z
			...
			function R(aa, Z) {
				...
				if (ac && typeof ac === "object"
						&& typeof ac.toJSON === "function") {
					ac = ac.toJSON(aa);
					ab = typeof ac
				}
				if (/(number|float|int|integer|string|boolean)/.test(ab)
						|| (!ac && ab === "object")) {
					return V(ac, ab)
				} else {
					return S(ac)
						...
						if (Q !== T) {
							...
							if (typeof H === "function") {
								...
								var X = function(ad, ac) {
									Object.getOwnPropertyNames(ad).forEach(
											function(ae) {
												var af = i.type(ad[ae]);
												if (af == "array"
														|| af == "object") {
	...
	i.jqx.dataAdapter = function(J, e) {
		...
		if (J != undefined && J.localdata != null
				&& typeof J.localdata == "function") {
			var I = J.localdata();
			if (I != null) {
				...
				if (J._localdata.subscribe) {
					...
					J._localdata.subscribe(function(K) {
								...
								if (H.suspendKO == false
										|| H.suspendKO == undefined
										|| H._oldlocaldata.length == 0) {
									H._oldlocaldata = L(K)
								}
	...
	i.jqx.dataAdapter.prototype = {
		...
		dataBind : function(R, aa) {
			...
			this.records = new Array();
			var K = this._options || {};
			this.virtualmode = K.virtualmode != undefined
					? K.virtualmode
					: false;
			this.totalrecords = K.totalrecords != undefined
					? K.totalrecords
					: 0;
			...
			this.cachedrecords = K.cachedrecords != undefined
					? K.cachedrecords
					: new Array();
			...
			var ah = this;
			var Q = W.datatype;
			if (W.datatype === "csv" || W.datatype === "tab"
					|| W.datatype === "tsv" || W.datatype == "text") {
				Q = "text"
			}
			...
			switch (Q) {
				...
				default :
					...
					this.totalrecords = this.virtualmode
							? (W.totalrecords || M)
							: M;
					if (this.unboundmode) {
						this.totalrecords = this.unboundmode
								? (W.totalrecords || M)
								: M;
							...
							if (typeof ap === "string") {
								...
								i.each(this, function(ay, aD) {
									...
									if (av) {
										...
										if (an[ay].subscribe) {
											var aA = am;
											ah.koSubscriptions[ah.koSubscriptions.length] = an[ay]
													.subscribe(function(aI) {
														var aH = aA;
														ak[ay] = aI;
														var aJ = {
															index : aH,
					...
					if (K.uniqueDataFields) {
						var S = this.getUniqueRecords(this.records,
								K.uniqueDataFields);
						this.records = S;
						this.cachedrecords = S
					}
					if (K.beforeLoadComplete) {
						var ae = K.beforeLoadComplete(ah.records,
								this.originaldata);
						if (ae != undefined) {
							ah.records = ae;
							ah.cachedrecords = ae
					...
					if (K.autoSort && K.autoSortField) {
						...
						Object.prototype.toString = (typeof field == "function")
								? field
								: function() {
						...
						ah.records.sort(function(ak, aj) {
									...
									if (i.jqx.dataFormat.isNumber(ak)
											&& i.jqx.dataFormat.isNumber(aj)) {
										if (ak < aj) {
											return -1
										}
										if (ak > aj) {
										...
										if (i.jqx.dataFormat.isDate(ak)
												&& i.jqx.dataFormat.isDate(aj)) {
											if (ak < aj) {
												return -1
											}
											if (ak > aj) {
				...
				case "ics" :
					if (W.localdata != null && !W.url) {
						...
						if (K.uniqueDataFields) {
							var S = ah.getUniqueRecords(ah.records,
									K.uniqueDataFields);
							ah.records = S;
							ah.cachedrecords = S
						}
						...
						if (i.isFunction(K.loadServerData)) {
							...
							this.xhr = i.jqx.data.ajax({
								...
								success : function(am, aj, ap) {
									...
									ah.addForeignValues(W);
									if (K.uniqueDataFields) {
										var al = ah.getUniqueRecords(
												ah.records, K.uniqueDataFields);
										ah.records = al;
										ah.cachedrecords = al
									}
									if (K.beforeLoadComplete) {
										var an = K.beforeLoadComplete(
												ah.records, am);
										if (an != undefined) {
											ah.records = an;
											ah.cachedrecords = an
		...
		buildHierarchy : function(K) {
			...
			if (e.hierarchy.root) {
				if (e.dataType == "xml") {
					var P = this.getRecordsHierarchy("uid", "parentuid",
							"records", null, K);
					this.hierarchy = P;
					return P
				} else {
			...
			if (e.hierarchy.keyDataField && e.hierarchy.parentDataField) {
				var P = this.getRecordsHierarchy(e.hierarchy.keyDataField.name,
						e.hierarchy.parentDataField.name, "records", null, K);
				this.hierarchy = P;
				return P
			}
			if (e.hierarchy.groupingDataFields) {
				...
				var P = this.getGroupedRecords(J, "records", "label", null,
						"data", null, "parent", K);
				this.hierarchy = P;
				return P
			}
			...
			if (H != undefined) {
				if (M != undefined) {
						...
						var K = function(O) {
							if (O) {
								for (var P = 0; P < O.length; P++) {
									var Q = O[P];
									if (Q.uid == M) {
										var R = (J._source && J._source.hierarchy)
												? J._source.hierarchy.reservedNames
												: null;
										if (R == null) {
											R = N()
										}
					...
					if (this.hierarchy && this.hierarchy.length >= 0
							&& (this._source.hierarchy || I)) {
						var L = (J._source && J._source.hierarchy)
								? J._source.hierarchy.reservedNames
								: null;
						if (L == null) {
							L = N()
		...
		getAggregatedData : function(S, P, M, H) {
					...
					if (I.aggregates) {
						...
						if (typeof U === "number" && isFinite(U)) {
							i.each(I.aggregates, function() {
								...
								if (this == "sum" || this == "avg"
										|| this == "stdev" || this == "stdevp"
										|| this == "var" || this == "varp") {
									W += parseFloat(U)
								} else {
									if (this == "product") {
			...
			for (var N = 0; N < S.length; N++) {
				...
				if (Q[I.name]["stdev"] || Q[I.name]["stdevp"]
						|| Q[I.name]["var"] || Q[I.name]["varp"]) {
					i.each(I.aggregates, function(ac) {
								if (this == "stdev" || this == "var"
										|| this == "varp" || this == "stdevp") {
					...
					i.each(I.aggregates, function(X) {
								...
								if (W) {
									if (this == "min" || this == "max"
											...
											|| this == "var" || this == "varp") {
										var Y = Q[I.name][this];
										Q[I.name][this] = i.jqx.dataFormat
												.formatnumber(Y, W, P)
									} else {
										if (typeof this == "object") {
											for (obj in this) {
												var Y = Q[I.name][obj];
												Q[I.name][obj] = i.jqx.dataFormat
														.formatnumber(Y, W, P)
											}
										}
									}
		...
		getRecordsHierarchy : function(L, J, aa, U, H) {
			...
			var ab = I;
			var O = ab.length;
			var P = (this._source && this._source.hierarchy)
					? this._source.hierarchy.reservedNames
					: null;
			var W = function(ac) {
				var ad = ac;
				if (U) {
		...
		getid : function(N, H, K) {
			...
			if (N) {
				if (N.toString().length > 0) {
					var e = i(H).attr(N);
					if (e != null && e.toString().length > 0) {
						return i.trim(e).split(" ").join("").replace(
								/([ #;?%&,.+*~\':"!^$[\]()=>|\/@])/g, "")
					} else {
		...
		loadjson : function(ae, af, R) {
			...
			if (ae == null || ae == undefined || ae == "undefined"
					|| ae.length == undefined) {
				console
						.log("jqxDataAdapter: JSON Parse error! Invalid JSON. Please, check your JSON or your jqxDataAdapter initialization!");
				return
			}
			if (ae.length == 0) {
				this.totalrecords = 0;
			...
			this.originaldata = new Array();
			var W = this.records;
			var T = !this.pageable ? R.recordstartindex : this.pagesize
					* this.pagenum;
			this.recordids = new Array();
			if (R.loadallrecords) {
				T = 0;
			...
			var P = 0;
			if (this.virtualmode) {
				T = !this.pageable ? R.recordstartindex : this.pagesize
						* this.pagenum;
				P = T;
				T = 0;
				J = this.totalrecords
		...
		loadxml : function(K, ai, U) {
			...
			this.originaldata = new Array();
			var aa = this.records;
			var X = !this.pageable ? U.recordstartindex : this.pagesize
					* this.pagenum;
			this.recordids = new Array();
			if (U.loadallrecords) {
				X = 0;
			...
			var S = 0;
			if (this.virtualmode) {
				X = !this.pageable ? U.recordstartindex : this.pagesize
						* this.pagenum;
				S = X;
				X = 0;
				J = this.totalrecords
				...
				if (!this.recordids[ah]) {
					...
					for (var Y = 0; Y < ac; Y++) {
						...
						if (W == "") {
							...
							if (W == "") {
								if (I.nodeName && I.nodeName == Q.name
										&& I.firstChild) {
			...
			if (U.hierarchy && U.hierarchy.root) {
				for (var Z = X; Z < J; Z++) {
					...
					if (i(I).parent().length > 0) {
						var ah = this
								.getid(U.id, i(I).parents(U.hierarchy.record
												+ ":first"));
						N.parentuid = ah
					} else {
						N.parentuid = null
		...
		loadics : function(N, H) {
			...
			var P = function(V) {
				...
				return new Date(Date.UTC(U[1], U[2] - 1, U[3], U[5] || 0, U[6]
								|| 0, U[7] || 0))
			};
			for (var M = 0; M < K; M++) {
				var O = R[M];
		...
		loadtext : function(Y, P) {
			...
			this.originaldata = new Array();
			var U = this.records;
			var R = !this.pageable ? P.recordstartindex : this.pagesize
					* this.pagenum;
			this.recordids = new Array();
			if (P.loadallrecords) {
				R = 0;
			...
			var N = 0;
			if (this.virtualmode) {
				R = !this.pageable ? P.recordstartindex : this.pagesize
						* this.pagenum;
				N = R;
				R = 0;
				J = this.totalrecords
	...
	i.extend(i.jqx.dataFormat, {
		...
		isValue : function(e) {
			return (this.isObject(e) || this.isString(e) || this.isNumber(e) || this
					.isBoolean(e))
		},
		isEmpty : function(e) {
		...
		getDayIndex : function(K, J, H) {
			...
			if (!I) {
				K._upperDays = I = [this.toUpperArray(L.names),
						this.toUpperArray(L.namesAbbr),
						this.toUpperArray(L.namesShort)]
			}
			J = J.toUpperCase();
			if (H) {
		...
		getMonthIndex : function(N, M, I) {
			...
			if (!K) {
				N._upperMonths = K = [this.toUpperArray(e.names),
						this.toUpperArray(e.namesAbbr)];
				N._upperMonthsGen = L = [this.toUpperArray(H.names),
						this.toUpperArray(H.namesAbbr)]
			}
			M = this.toUpper(M);
		...
		tryparsedate : function(ai, aj) {
			...
			if (ai != null && ai.substring(0, 6) == "/Date(") {
				...
				if (ap == "Invalid Date") {
					...
					var ap = null;
					if (al) {
						ap = new Date(1 * al[1] + 3600000 * al[2] + 60000
								* al[3])
					}
				}
			...
			if (i.type(ai) === "string") {
				...
				if (ai.indexOf(":") >= 0) {
					...
					if (ai.toUpperCase().indexOf("AM") >= 0) {
						...
						if (ai.toUpperCase().indexOf("PM") >= 0) {
							I = ai
									.substring(ai.toUpperCase().indexOf("PM")
											- 2);
							I = ae.trim(I);
							W = ai.substring(0, ai.toUpperCase().indexOf("PM")
											- 2)
						}
					}
				}
				...
				if (W) {
					...
					for (var ao = 0; ao < L.length; ao++) {
						var N = L[ao];
						var ab = ae.parsedate(N, "d", aj)
								|| ae.parsedate(N, "dd", aj)
								|| ae.parsedate(N, "ddd", aj)
								|| ae.parsedate(N, "dddd", aj);
						if (ab) {
							if (N.length <= 2) {
								Q = ao;
								H.push(ab.getDate());
					...
					for (var ao = 0; ao < L.length; ao++) {
						var N = L[ao];
						var S = ae.parsedate(N, "M", aj)
								|| ae.parsedate(N, "MM", aj)
								|| ae.parsedate(N, "MMM", aj)
								|| ae.parsedate(N, "MMMM", aj);
						if (S) {
							if (Q != undefined && Q == ao) {
								continue
							}
				...
				if (I) {
					...
					var K = ae.parsedate(I, "h:mm tt", aj)
							|| ae.parsedate(I, "h:mm:ss tt", aj)
							|| ae.parsedate(I, "HH:mm:ss.fff", aj)
							|| ae.parsedate(I, "HH:mm:ss.ff", aj)
							|| ae.parsedate(I, "HH:mm:ss.tttt", aj)
							|| ae.parsedate(I, "HH:mm:ss", aj)
							|| ae.parsedate(I, "HH:mm", aj)
							|| ae.parsedate(I, "HH", aj);
					var V = 0, J = 0, Y = 0, Z = 0;
					if (K && K.getTime() != NaN) {
						V = K.getHours();
						J = K.getMinutes();
		...
		getparseregexp : function(e, R) {
			...
			var Q = this.expandFormat(e, R).replace(
					/([\^\$\.\*\+\?\|\[\]\(\)\{\}])/g, "\\\\$1"), O = ["^"], H = [], N = 0, J = 0, W = this
					.getTokenRegExp(), L;
		...
		expandYear : function(L, J) {
			...
			if (J < 100) {
				...
				I = typeof I === "string" ? new Date().getFullYear() % 100
						+ parseInt(I, 10) : I;
		...
		parsedate : function(ab, ai, W) {
			...
			ab = this.trim(ab);
			var T = W, an = this.getparseregexp(T, ai), N = new RegExp(an.regExp)
					.exec(ab);
			if (N === null) {
				return null
			}
		...
		formatdate : function(Z, ad, U) {
			...
			af = [];
			var M, ab = ["0", "00", "000"], Q, R, e = /([^d]|^)(d|dd)([^d]|$)/g, ae = 0, W = this
					.getTokenRegExp(), I;
			function O(ag, aj) {
				var ai, ah = ag + "";
			...
			for (;;) {
				...
				switch (X) {
					...
					case "t" :
					case "tt" :
						Y = Z.getHours() < 12 ? (U.AM ? U.AM[0] : " ") : (U.PM
								? U.PM[0]
								: " ");
					...
					case "zz" :
						...
						af.push((M <= 0 ? "+" : "-")
								+ O(Math.floor(Math.abs(M)), L));
						break;
					case "zzz" :
						M = Z.getTimezoneOffset() / 60;
						af.push((M <= 0 ? "+" : "-")
								+ O(Math.floor(Math.abs(M)), 2) + ":"
								+ O(Math.abs(Z.getTimezoneOffset() % 60), 2));
						break;
					case "g" :
					case "gg" :
						if (U.eras) {
	...
	i.extend(i.jqx.data, {
		...
		ajax : function(M, J) {
			...
			J = J || {};
			var P, ad, K, Y, R, V, I, X, Q = i.jqx.data.ajaxSetup({}, J), af = Q.context
					|| Q, T = af !== Q && (af.nodeType || af instanceof i)
					? i(af)
					: i.event, ae = i.Deferred(), aa = i
					.Callbacks("once memory"), N = Q.statusCode || {}, U = {}, ab = {}, L = 0, O = "canceled", W = {
				readyState : 0,
			...
			function S(ak, ag, al, ai) {
				...
				N = undefined;
				if (I) {
					T.trigger("ajax" + (e ? "Success" : "Error"), [W, Q,
									e ? ao : am])
				}
				aa.fireWith(af, [W, ah]);
				if (I) {
			...
			if (Q.crossDomain == null) {
				...
				Q.crossDomain = !!(V && (V[1] !== l[1] || V[2] !== l[2] || (V[3] || (V[1] === "http:"
						? 80
						: 443)) != (l[3] || (l[1] === "http:" ? 80 : 443))))
			}
			...
			if (!Q.hasContent) {
				...
				if (Q.cache === false) {
					...
					Q.url = ac
							+ ((ac === Q.url) ? (k.test(Q.url) ? "&" : "?")
									+ "_=" + H : "")
				}
			}
			if (Q.data && Q.hasContent && Q.contentType !== false
					|| J.contentType) {
			...
			W.setRequestHeader("Accept", Q.dataTypes[0]
			...
			if (Q.beforeSend
					&& (Q.beforeSend.call(af, W, Q) === false || L === 2)) {
				return W.abort()
			}
			O = "abort";
			for (X in {
					...
					if (P !== true) {
						if (P && R["throws"]) {
							...
							try {
								...
								return {
									state : "parsererror",
									error : P ? M : "No conversion from " + I
											+ " to " + N
								}
							}
						}
	...
	i.jqx.data.ajaxPrefilter("json jsonp", function(Q, L, P) {
				var O, e, N, J = Q.data, H = Q.url, I = Q.jsonp !== false, M = I
						&& D.test(H), K = I
						&& !M
						&& typeof J === "string"
						&& !(Q.contentType || "")
				...
				if (Q.dataTypes[0] === "jsonp" || M || K) {
					O = Q.jsonpCallback = i.isFunction(Q.jsonpCallback) ? Q
							.jsonpCallback() : Q.jsonpCallback;
					e = window[O];
					if (M) {
					...
					if (!e.crossDomain || i.support.cors) {
						...
						return {
							send : function(N, I) {
								var L, K, M = e.xhr();
								if (e.username) {
									M.open(e.type, e.url, e.async, e.username,
											e.password)
								} else {
	...
	i.jqx.filter = function() {
		...
		var M = 0;
		var J = 1;
		var P = ["EMPTY", "NOT_EMPTY", "CONTAINS", "CONTAINS_CASE_SENSITIVE",
				"DOES_NOT_CONTAIN", "DOES_NOT_CONTAIN_CASE_SENSITIVE",
				"STARTS_WITH", "STARTS_WITH_CASE_SENSITIVE", "ENDS_WITH",
				"ENDS_WITH_CASE_SENSITIVE", "EQUAL", "EQUAL_CASE_SENSITIVE",
				"NULL", "NOT_NULL"];
		var R = ["EQUAL", "NOT_EQUAL", "LESS_THAN", "LESS_THAN_OR_EQUAL",
				"GREATER_THAN", "GREATER_THAN_OR_EQUAL", "NULL", "NOT_NULL"];
		var S = ["EQUAL", "NOT_EQUAL", "LESS_THAN", "LESS_THAN_OR_EQUAL",
				"GREATER_THAN", "GREATER_THAN_OR_EQUAL", "NULL", "NOT_NULL"];
		...
		var T = function(V, U) {
			...
			this.evaluate = function(af) {
				...
				var Z = new Array();
				if (ae && ae.indexOf) {
					if (ae.indexOf("|") >= 0 || ae.indexOf(" AND ") >= 0
							|| ae.indexOf(" OR ") >= 0
							|| ae.indexOf(" and ") >= 0
							|| ae.indexOf(" or ") >= 0) {
						var aa = am(an, ae);
						...
						var ab = ae.indexOf(" AND ") >= 0
								? ae.split(" AND ")
								: new Array();
						var Y = ae.indexOf(" OR ") >= 0
								? ae.split(" OR ")
								: new Array();
						var X = ae.indexOf(" and ") >= 0
								? ae.split(" and ")
								: new Array();
						var ac = ae.indexOf(" or ") >= 0
								? ae.split(" or ")
								: new Array();
						var W = ae.indexOf("|") >= 0
								? ae.split("|")
		...
		var N = function(V, U) {
			...
			this.evaluate = function(ag) {
				...
				var am = function(ap, ao) {
					switch (al) {
						...
						case "LESS_THAN_OR_EQUAL" :
							return ap <= ao;
						case "STARTS_WITH" :
							return i.jqx.string.startsWithIgnoreCase(ap
											.toString(), ao.toString());
						case "ENDS_WITH" :
							return i.jqx.string.endsWithIgnoreCase(ap
											.toString(), ao.toString());
						case "ENDS_WITH_CASE_SENSITIVE" :
							return i.jqx.string.endsWith(ap.toString(), ao
											.toString());
						case "STARTS_WITH_CASE_SENSITIVE" :
							return i.jqx.string.startsWith(ap.toString(), ao
											.toString());
						case "CONTAINS" :
							return i.jqx.string.containsIgnoreCase(ap
											.toString(), ao.toString());
						case "CONTAINS_CASE_SENSITIVE" :
							return i.jqx.string.contains(ap.toString(), ao
											.toString());
						case "DOES_NOT_CONTAIN" :
							return !i.jqx.string.containsIgnoreCase(ap
											.toString(), ao.toString());
				...
				var aa = new Array();
				if (af && af.indexOf) {
					if (af.indexOf("|") >= 0 || af.indexOf(" AND ") >= 0
							|| af.indexOf(" OR ") >= 0
							|| af.indexOf(" and ") >= 0
							|| af.indexOf(" or ") >= 0) {
						var ab = am(an, af);
						...
						af = af.toString();
						var ac = af.indexOf(" AND ") >= 0
								? af.split(" AND ")
								: new Array();
						var Z = af.indexOf(" OR ") >= 0
								? af.split(" OR ")
								: new Array();
						var Y = af.indexOf(" and ") >= 0
								? af.split(" and ")
								: new Array();
						var ad = af.indexOf(" or ") >= 0
								? af.split(" or ")
								: new Array();
						ac = ac.concat(Y);
						Z = Z.concat(ad);
		...
		var H = function(X, V, W, ab) {
			...
			this.evaluate = function(ao) {
				...
				try {
					...
					var at = false;
					if (W != undefined && ab != undefined) {
						if (W.indexOf("t") >= 0 || W.indexOf("T") >= 0
								|| W.indexOf(":") >= 0 || W.indexOf("f") >= 0) {
							at = true;
							if (an && an.toString().indexOf(":") == -1) {
								var aj = i.jqx.dataFormat.tryparsedate(an
												.toString()
												+ ":00", ab);
								if (aj != null) {
									Z.filterdate = aj
				...
				if (at) {
					if (an && an.setFullYear) {
						if (ay && ay.getFullYear) {
							if (W.indexOf("d") == -1 && W.indexOf("M") == -1
									&& W.indexOf("y") == -1) {
								an.setFullYear(ay.getFullYear(), ay.getMonth(),
										ay.getDate())
							}
						}
					}
				...
				var ax = function(aA, az) {
					...
					switch (aw) {
						...
						case "LESS_THAN_OR_EQUAL" :
							return aA <= az;
						case "STARTS_WITH" :
							return i.jqx.string.startsWithIgnoreCase(aA
											.toString(), az.toString());
						case "ENDS_WITH" :
							return i.jqx.string.endsWithIgnoreCase(aA
											.toString(), az.toString());
						case "ENDS_WITH_CASE_SENSITIVE" :
							return i.jqx.string.endsWith(aA.toString(), az
											.toString());
						case "STARTS_WITH_CASE_SENSITIVE" :
							return i.jqx.string.startsWith(aA.toString(), az
											.toString());
						case "CONTAINS" :
							return i.jqx.string.containsIgnoreCase(aA
											.toString(), az.toString());
						case "CONTAINS_CASE_SENSITIVE" :
							return i.jqx.string.contains(aA.toString(), az
											.toString());
						case "DOES_NOT_CONTAIN" :
							return !i.jqx.string.containsIgnoreCase(aA
											.toString(), az.toString());
				...
				var ag = new Array();
				if (an && an.indexOf) {
					if (an.indexOf("|") >= 0 || an.indexOf(" AND ") >= 0
							|| an.indexOf(" OR ") >= 0
							|| an.indexOf(" and ") >= 0
							|| an.indexOf(" or ") >= 0) {
						var aj = ax(ay, an);
						...
						var ak = an.indexOf(" AND ") >= 0
								? an.split(" AND ")
								: new Array();
						var af = an.indexOf(" OR ") >= 0
								? an.split(" OR ")
								: new Array();
						var ae = an.indexOf(" and ") >= 0
								? an.split(" and ")
								: new Array();
						var al = an.indexOf(" or ") >= 0
								? an.split(" or ")
								: new Array();
						ak = ak.concat(ae);
						af = af.concat(al);
	...
	i.jqx.longInt = function() {
				...
				if (I <= -e.mathObj.Long.TWO_PWR_63_DBL_) {
					...
					if (I + 1 >= e.mathObj.Long.TWO_PWR_63_DBL_) {
						...
						if (I < 0) {
							...
							return new e.mathObj.Long(
									(I % e.mathObj.Long.TWO_PWR_32_DBL_) | 0,
									(I / e.mathObj.Long.TWO_PWR_32_DBL_) | 0)
						}
					}
				}
		...
		e.mathObj.Long.TWO_PWR_32_DBL_ = e.mathObj.Long.TWO_PWR_16_DBL_
				* e.mathObj.Long.TWO_PWR_16_DBL_;
		e.mathObj.Long.TWO_PWR_31_DBL_ = e.mathObj.Long.TWO_PWR_32_DBL_ / 2;
		e.mathObj.Long.TWO_PWR_48_DBL_ = e.mathObj.Long.TWO_PWR_32_DBL_
				* e.mathObj.Long.TWO_PWR_16_DBL_;
		e.mathObj.Long.TWO_PWR_64_DBL_ = e.mathObj.Long.TWO_PWR_32_DBL_
				* e.mathObj.Long.TWO_PWR_32_DBL_;
		...
		e.mathObj.Long.MAX_VALUE = e.mathObj.Long.fromBits(4294967295 | 0,
				2147483647 | 0);
		...
		e.mathObj.Long.prototype.toNumber = function() {
			return this.highBits * e.mathObj.Long.TWO_PWR_32_DBL_
		...
		e.mathObj.Long.prototype.getLowBitsUnsigned = function() {
			return (this.lowBits >= 0)
					? this.lowBits
					: e.mathObj.Long.TWO_PWR_32_DBL_ + this.lowBits
		};
		...
		e.mathObj.Long.prototype.equals = function(I) {
			return (this.highBits === I.highBits)
					&& (this.lowBits === I.lowBits)
		};
		...
		e.mathObj.Long.prototype.multiply = function(P) {
			...
			if (this.lessThan(e.mathObj.Long.TWO_PWR_24_)
					&& P.lessThan(e.mathObj.Long.TWO_PWR_24_)) {
				return e.mathObj.Long
						.fromNumber(this.toNumber() * P.toNumber())
			}
			var N = this.highBits >>> 16;
			var J = this.highBits & 65535;
			var Q = this.lowBits >>> 16;
		...
		e.mathObj.Long.prototype.div = function(K) {
			...
			var N, P;
			if (this.equals(e.mathObj.Long.MIN_VALUE)) {
				if (K.equals(e.mathObj.Long.ONE)
						|| K.equals(e.mathObj.Long.NEG_ONE)) {
					return e.mathObj.Long.MIN_VALUE
				} else {
		...
		e.mathObj.Long.prototype.and = function(I) {
			return e.mathObj.Long.fromBits(this.lowBits & I.lowBits,
					this.highBits & I.highBits)
		};
		...
		e.mathObj.Long.prototype.shiftLeft = function(K) {
			...
			if (K === 0) {
				...
				if (K < 32) {
					var J = this.highBits;
					return e.mathObj.Long.fromBits(I << K, (J << K)
									| (I >>> (32 - K)))
				} else {
		...
		e.mathObj.Long.prototype.shiftRight = function(K) {
			...
			if (K === 0) {
				...
				if (K < 32) {
					var I = this.lowBits;
					return e.mathObj.Long.fromBits((I >>> K) | (J << (32 - K)),
							J >> K)
				} else {
					return e.mathObj.Long.fromBits(J >> (K - 32), J >= 0
									? 0
									: -1)
				}
			}
