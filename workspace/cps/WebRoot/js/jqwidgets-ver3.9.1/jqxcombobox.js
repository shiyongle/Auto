	...
	a.extend(a.jqx._jqxComboBox.prototype, {
		defineInstance : function() {
			var b = {
				...
				events : ["open", "close", "select", "unselect", "change",
						"checkChange", "bindingComplete"]
			};
		...
		render : function() {
					...
					rendered : function() {
						...
						if (n.remoteAutoComplete) {
							if (n.autoDropDownHeight) {
								n.container.height(n.listBox.virtualSize.height
										+ 25);
								n.listBoxContainer
								...
								n.container.height(n.listBoxContainer.height()
										+ 25)
							}
			...
			n.propertyChangeMap.disabled = function(e, u, t, v) {
				if (v) {
					...
					e.dropdownlistContent.addClass(r
							.toThemeProperty("jqx-combobox-content-disabled"))
				} else {
					e.host.removeClass(r
							.toThemeProperty("jqx-combobox-state-disabled"));
					e.host.removeClass(r
							.toThemeProperty("jqx-fill-state-disabled"));
					e.dropdownlistContent.removeClass(r
							.toThemeProperty("jqx-combobox-content-disabled"))
				}
		...
		_search : function(m) {
			...
			if (d.multiSelect) {
				...
				var f = parseInt(this._findPos(d.host[0])[1])
						+ parseInt(d.host.outerHeight()) - 1 + "px";
		...
		_updateItemsVisibility : function(l) {
						...
						if (!this.isGroup) {
							...
							if (j.multiSelect) {
								...
								if (j.selectedItems.indexOf(this.value) >= 0
										|| j._disabledItems.indexOf(this.value) >= 0) {
									this.disabled = true;
									q = false
								}
							}
			...
			if (this.autoDropDownHeight) {
				...
				if (n < parseInt(this.dropDownHeight)) {
					var c = this.listBox.hScrollBar[0].style.visibility == "hidden"
							? 0
							: 20;
		...
		isOpened : function() {
			var c = this;
			var b = a.data(document.body, "openedCombojqxListBox"
							+ this.element.id);
		...
		_updateHandlers : function() {
			...
			if (!this.touch) {
				if (this.host.parents()) {
					this.addHandler(this.host.parents(), "scroll.combobox"
									+ this.element.id, function(f) {
								var g = d.isOpened();
								if (g) {
									d.close()
								}
			...
			if (d.autoOpen) {
				...
				this.addHandler(a(document), "mousemove." + d.id, function(i) {
					...
					if (h && d.autoOpen) {
						...
						if (i.pageY >= g
								&& i.pageY <= g + d.container.height() - 20) {
							if (i.pageX >= f
									&& i.pageX < f + d.container.width()) {
								canClose = false
							}
						}
						if (canClose) {
					...
					if (!d.isanimating) {
						if (f) {
							...
							if (d.autoDropDownHeight) {
								d.container.height(d.listBoxContainer.height()
										+ 25);
			...
			this.addHandler(this.input, "focus", function(f) {
						...
						d.dropdownlistContent.addClass(d
			...
			this.addHandler(a(document), "mousedown." + this.id,
				...
				if (k.keyCode == "13" || k.keyCode == "9") {
					if (h && !d.isanimating) {
						if (d.listBox.selectedIndex != -1) {
							...
							d._setSelection(d.input.val().length,
									d.input.val().length);
							d.hideListBox("keyboard")
						}
						if (k.keyCode == "13") {
		...
		removeHandlers : function() {
			...
			if (this.host.parents()) {
				this.removeHandler(this.host.parents(), "scroll.combobox"
								+ this.element.id)
			}
		...
		enableAt : function(b) {
			var c = this.getVisibleItem(b);
			if (c) {
				this._disabledItems.splice(
						this._disabledItems.indexOf(c.value), 1)
			}
		...
		enableItem : function(b) {
			...
			if (b) {
				this._disabledItems.splice(
						this._disabledItems.indexOf(b.value), 1)
			}
		...
		testOffset : function(h, f, c) {
			...
			f.top -= Math.min(f.top, (f.top + j > e && e > j) ? Math.abs(j + c
							+ 23) : 0);
			return f
		},
		open : function() {
		...
		showListBox : function(l) {
			...
			if (this.dropDownWidth == "auto" && this.width != null
					&& this.width.indexOf && this.width.indexOf("%") != -1) {
			...
			var m = parseInt(this._findPos(this.host[0])[1])
					+ parseInt(this.host.outerHeight()) - 1 + "px";
			...
			this.host
					.addClass(this.toThemeProperty("jqx-combobox-state-focus"));
			...
			var vtop = object.offsetTop;
			var htop =this.cinput.parents('html').height();
			var listTop = this.container.height();
			while(object.offsetParent){
				vtop +=  object.offsetParent.offsetTop;
				//object =  object.element;2016年3月23日15:12:55 HT注释这句
				object =  object.offsetParent;//2016年3月23日15:13:38 HT
				if(!object){
					break;
				}
			}
			if(vtop+listTop>htop){//iframe里下拉框超出时，向上显示
				this.container.css("top", parseInt(m)-listTop+"px");
			}
			v._arrange();
			var c = true;
			var b = false;
			if (this.dropDownHorizontalAlignment == "right" || this.rtl) {
				...
				if (k > this.host.width()) {
					this.container.css("left", 25 + parseInt(Math.round(q)) - s
									+ "px")
				} else {
					this.container.css("left", 25 + parseInt(Math.round(q)) + s
									+ "px")
			...
			if (this.animationType == "none") {
				...
				a
						.data(document.body, "openedCombojqxListBox"
										+ o.element.id, h);
				h.css("margin-top", 0);
				h.css("opacity", 1)
			} else {
		...
		doMultiSelect : function(c) {
			...
			for (var b = 0; b < f.length; b++) {
				...
				if (this.selectedItems.indexOf(f[b].value) >= 0
						|| this._disabledItems.indexOf(this.value) >= 0) {
					f[b].disabled = true
				}
			}
			this.listBox._renderItems();
			...
			a.each(this.selectedItems, function(l) {
				...
				if (n) {
					...
					if (p[0].innerHTML == "") {
						p[0].innerHTML = '<a data-value="' + n.value
								+ '" style="float: left;" href="#">' + q
								+ "</a>"
					}
					if (e.rtl) {
						p[0].innerHTML = '<a data-value="' + n.value
								+ '" style="float: right;" href="#">' + q
								+ "</a>"
					}
			...
			this.addHandler(g.find(".jqx-icon-close"), d, function(p) {
						...
						var r = a(p.target).parent().parent().find("a")
								.attr("data-value");
						var o = e.getItemByValue(r);
						if (o
								|| (e.remoteAutoComplete && !o && e.selectedItems
										.indexOf(r) >= 0)) {
							e.listBox.selectedValue = null;
		...
		hideListBox : function(h) {
			...
			var d = this;
			a.data(document.body, "openedCombojqxListBox" + this.element.id,
					null);
			if (this.animationType == "none") {
				this.opening = false;
			...
			this.host.removeClass(this
					.toThemeProperty("jqx-combobox-state-selected"));
			this.dropdownlistArrowIcon.removeClass(this
					.toThemeProperty("jqx-icon-arrow-down-selected"));
			...
			if (!this.focused) {
				...
				this.host.removeClass(this
						.toThemeProperty("jqx-fill-state-focus"));
				this.dropdownlistContent.removeClass(this
						.toThemeProperty("jqx-combobox-content-focus"))
			}
		...
		_arrange : function() {
			...
			if (this.rtl) {
				...
				this.dropdownlistContent.css("left", this.dropdownlistArrow
								.width());
				if (a.jqx.browser.msie && a.jqx.browser.version <= 8) {
					this.dropdownlistContent.css("left", 1
