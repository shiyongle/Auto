	...
	a.extend(a.jqx._jqxListBox.prototype, {
		defineInstance : function() {
			var b = {
				...
				events : ["select", "unselect", "change", "checkChange",
						"dragStart", "dragEnd", "bindingComplete"]
			};
		...
		render : function() {
			...
			m.filterInput
					.addClass(m
							.toThemeProperty("jqx-widget jqx-listbox-filter-input jqx-input jqx-rc-all"));
			...
			m.bottomRight = a(m.element.firstChild.firstChild.firstChild.nextSibling.nextSibling.nextSibling.nextSibling)
					.addClass(m.toThemeProperty("jqx-listbox-bottomright"))
					.addClass(m.toThemeProperty("jqx-scrollbar-state-normal"));
		...
		_updateTouchScrolling : function() {
			...
			if (this.isTouchDevice()) {
				b.enableHover = false;
				var c = this.overlayContent
						? this.overlayContent
						: this.content;
				this.removeHandler(a(c), a.jqx.mobile
				...
				a.jqx.mobile.touchScroll(c, b.vScrollInstance.max, function(f,
								e) {
		...
		_hitTest : function(c, f) {
			...
			var b = this._searchFirstVisibleIndex(f + e,
					this.renderedVisibleItems);
			if (this.renderedVisibleItems[b] != undefined
					&& this.renderedVisibleItems[b].isGroup) {
				return null
			}
			if (this.renderedVisibleItems.length > 0) {
				var d = this.renderedVisibleItems[this.renderedVisibleItems.length
						- 1];
		...
		_renderItems : function() {
			...
			var m = g > 0 ? this._searchFirstVisibleIndex(
					this.vScrollInstance.value, this.renderedVisibleItems) : 0;
			var O = 0;
			F = 0;
			...
			var o = this.toThemeProperty("jqx-listitem-state-normal") + " "
					+ this.toThemeProperty("jqx-item");
			var i = this.toThemeProperty("jqx-listitem-state-group");
			var P = this.toThemeProperty("jqx-listitem-state-disabled") + " "
			...
			for (var r = 0; r < this.visualItems.length; r++) {
				...
				if (r < L.length) {
					...
					if (N) {
						p += " "
								+ this
										.toThemeProperty("jqx-listitem-state-normal-touch")
					}
					A[0].className = p;
					if (this.renderer) {
						if (!x.key) {
					...
					if (this.equalItemsWidth && !x.isGroup) {
						...
						if (K > this.virtualSize.width) {
							...
							A[0].style.width = -4 + this.virtualSize.width
									+ "px";
					...
					if (this.checkboxes && !x.isGroup) {
						...
						var j = x.checked
								...
								: "";
						if (e[0].firstChild && e[0].firstChild.firstChild
							...
							if (e[0].firstChild.firstChild) {
								if (u) {
									...
									if (u === false) {
										...
										if (u === null) {
											e[0].firstChild.firstChild.firstChild.className = this
					...
					if (!x.disabled
							&& ((!this.filterable && this.selectedIndexes[x.visibleIndex] >= 0) || (x.selected && this.filterable))) {
						A
								.addClass(this
										.toThemeProperty("jqx-listitem-state-selected"));
						A.addClass(this
		...
		_calculateVirtualSize : function(l) {
			...
			if (this.autoItemsHeight
				...
				for (var g = 0; g < e; g++) {
					...
					if (!s.isGroup) {
						d += this
								.toThemeProperty("jqx-listitem-state-normal jqx-rc-all")
					} else {
						d += this
								.toThemeProperty("jqx-listitem-state-group jqx-rc-all")
					}
						...
						if (s.html == null
							...
							if (s.html != null && s.html.toString().length > 0) {
								...
								if (s.label != null || s.value != null) {
									if (s.label != null) {
										if (s.label.toString()
												.match(new RegExp("\\w")) != null
												|| s.label
														.toString()
														.match(new RegExp("\\d")) != null) {
											o.innerHTML = s.label
										} else {
											o.innerHTML = "Item"
										}
			...
			for (var j = e; j < v.virtualItemsCount; j++) {
				...
				if (v.checkboxes) {
					...
					var o = '<div class="'
							...
							+ '"><div style="cursor: pointer; width: 13px; height: 13px;">';
					var w = u.checked
							? " "
									+ v
											.toThemeProperty("jqx-checkbox-check-checked")
							: "";
					o += '<span style="width: 13px; height: 13px;" class="checkBoxCheck'
							+ w + '"></span>';
					o += "</div></div>";
					i += o;
					i += "</div>"
		...
		_updatescrollbars : function() {
			...
			if (k.autoItemsHeight) {
				...
				if (j >= o - h - g) {
					...
					if (k.vScrollBar[0].style.visibility == "inherit") {
						f.max = 2 + parseInt(n) + k.hScrollBar.outerHeight()
								+ 2 - parseInt(k.host.height())
					}
				} else {
			...
			if (k.itemswrapper) {
				k.itemswrapper[0].style.width = Math.max(0, Math.max(o - 2, 17
										+ j))
						+ "px";
		...
		selectIndex : function(j, r, c, d, m, b) {
			...
			if (!this.multiple && !this.multipleextended
					&& this.selectedIndex == j && !d && !this.checkboxes) {
				if (this.visibleItems && this.items
			...
			if (d != undefined && d) {
				...
				var k = function(t, x, v, w, u, i) {
					if (l.selectedIndexes[t] == undefined
						...
						if (this._ctrlKey) {
							...
							if (this._shiftKey) {
								...
								for (var g = f; g <= n; g++) {
									...
									l.selectedValues[l.getVisibleItem(g).value] = l
											.getVisibleItem(g).value;
									l._raiseEvent("0", {
												index : g,
												type : m,
			...
			if (this.field && this.input) {
				if (this.field.nodeName.toLowerCase() == "select") {
					...
					a.each(this.items, function(f, g) {
								a(this.originalItem.originalItem)
										.removeAttr("data-selected");
								this.selected = c.indexOf(this.value) >= 0;
								if (this.selected) {
									a(this.originalItem.originalItem).attr(
											"data-selected", true)
								}
							})
				}
		...
		_itemsInPage : function() {
			...
			if (this.items) {
				a.each(this.items, function() {
							if ((this.initialTop + this.height) >= c.content
									.height()) {
								return false
							}
							b++
			...
			if (this.visibleItems != null) {
				if (this.visibleItems[this.visibleItems.length - 1]) {
					if (this.visibleItems[this.visibleItems.length - 1].isGroup) {
						return this
								._prevItemIndex(this.visibleItems.length - 1)
					} else {
		...
		_nextItemIndex : function(b) {
			for (indx = b + 1; indx < this.visibleItems.length; indx++) {
				if (this.visibleItems[indx]) {
					if (!this.visibleItems[indx].disabled
							&& !this.visibleItems[indx].isGroup) {
						return indx
					}
				}
			}
		...
		_search : function(c) {
			...
			if (b.searchMode == "none" || b.searchMode == null
		...
		_syncSelection : function() {
			...
			if (d.filterable) {
				...
				for (var b = 0; b < d.visibleItems.length; b++) {
					...
					if (d.selectedIndexes
							&& d.selectedIndexes[b] == c.visibleIndex) {
						c.selected = true
					}
				}
				if (d.itemswrapper) {
		...
		_handleKeyDown : function(n) {
			...
			if (k.incrementalSearch) {
				...
				if ((s == 8 || s == 46) && k._searchString.length >= 1) {
					k._searchString = k._searchString.substr(0,
				...
				if ((s >= 65 && s <= 97) || m || s == 8 || s == 32 || s == 46) {
					...
					if (o >= 0) {
						var h = function() {
							j();
							k
									.selectIndex(o, false, false, false,
											"keyboard", n);
			...
			if (s == 33) {
				...
				if (k.selectedIndex - p >= 0) {
					...
					j();
					k.selectIndex(k._firstItemIndex(), false, false, false,
							"keyboard", n)
				}
				k._searchString = ""
			}
			...
			if (s == 35) {
				j();
				k.selectIndex(k._lastItemIndex(), false, false, false,
						"keyboard", n);
				k._searchString = ""
			}
			if (s == 34) {
				...
				if (s == 40) {
					...
					if (k.selectedIndex + 1 < k.visibleItems.length) {
						...
						if (c != k.selectedIndex && c != -1) {
							j();
							k
									.selectIndex(c, false, false, false,
											"keyboard", n)
						} else {
							return true
						}
		...
		_updateItemCheck : function(b, c) {
			...
			if (b.checked == true) {
				b.checked = (b.hasThreeStates && this.hasThreeStates)
						? null
						: false
			} else {
		...
		_removeHandlers : function() {
			var b = this;
			this
					.removeHandler(a(document), "keydown.listbox"
									+ this.element.id);
			...
			if (this.overlayContent) {
				this.removeHandler(this.overlayContent, a.jqx.mobile
								.getTouchEventName("touchend"))
			}
		},
		_updateSize : function() {
			...
			var b = this;
			b._arrange();
			if (b.host.height() != b._oldheight
					|| b.host.width() != b._oldwidth) {
				...
				if (b.autoItemsHeight) {
					...
					if (b.items) {
						if (b.items.length > 0
								&& b.virtualItemsCount * b.items[0].height < b._oldheight
										- 2) {
							b._render(false)
						} else {
							var d = b.vScrollInstance.value;
							...
							if (d < b.vScrollInstance.max) {
								...
								b.vScrollInstance
										.setPosition(b.vScrollInstance.max)
							}
						}
					}
				}
		...
		_addHandlers : function() {
			...
			this.addHandler(this.content, "mouseleave", function(n) {
				...
				if (o != null) {
					a(o).removeClass(l
							.toThemeProperty("jqx-listitem-state-hover"));
			...
			if (this.enableSelection) {
				...
				if (this.overlayContent) {
					this.addHandler(this.overlayContent, a.jqx.mobile
									.getTouchEventName("touchend"),
							function(p) {
					...
					this.addHandler(this.content, i, function(n) {
						...
						if (n.target.id != ("listBoxContent" + l.element.id)
							...
							if (w != null && !w.isGroup) {
								var p = function(y, x) {
									...
									if (!l.checkboxes) {
										l.selectIndex(y.visibleIndex, false,
												true, false, "mouse", x)
									} else {
										v = 20 + x.pageX - z.left;
										if (l.rtl) {
			...
			if (this.enableHover && !e) {
				this._mousemovefunc = function(n) {
					...
					var p = a.jqx.browser.msie == true
							&& a.jqx.browser.version < 9 ? 0 : 1;
					if (n.target == null) {
						return true
					}
					if (l.disabled) {
		...
		_arrange : function(u) {
			...
			if ((t.vScrollBar[0].style.visibility != "hidden")
				...
				t.bottomRight[0].style.left = 1
						+ parseInt(t.vScrollBar[0].style.left) + "px";
				t.bottomRight[0].style.top = 1
						+ parseInt(t.hScrollBar[0].style.top) + "px";
				if (t.rtl) {
					t.bottomRight.css({
								left : 0
							})
			...
			if (g > 0) {
				...
				t.content[0].style.height = parseInt(t.content[0].style.height)
						- g + "px"
			}
			if (t.filterable) {
				...
				if (!t.rtl) {
					t.filterInput[0].style.left = parseInt(t.content
							.css("left"))
							+ 3 + "px"
				}
				t.filterInput[0].style.width = parseInt(t.content.css("width"))
						- 7 + "px";
				...
				if (this.autoHeight) {
					...
					for (indx = 0; indx < this.visibleItems.length; indx++) {
						...
						if (l.visibleIndex == h && !l.isGroup) {
							var g = a.data(this.vScrollBar[0], "jqxScrollBar").instance;
							var k = g.value;
							var b = !this.filterable ? 0 : this.filterHeight
									+ 2;
							...
							if (l.initialTop < k) {
								...
								if (l.initialTop + l.height > k
										+ this.host.height() - b) {
									var c = this.host.height();
									if (this.filterable) {
										g.setPosition(this.filterHeight + 2
												+ l.initialTop + l.height + 2
												- c + d)
									} else {
										g.setPosition(l.initialTop + l.height
												+ 2 - c + d);
										if (indx === this.visibleItems.length
												- 1) {
											g.setPosition(g.max)
										}
									}
		...
		loadItems : function(m, o) {
			...
			a.map(m, function(x) {
						...
						j.hasThreeStates = x.hasThreeStates != undefined
								? x.hasThreeStates
								: true;
						j.originalItem = x;
						if (o) {
			...
			if (this.fromSelect == undefined || this.fromSelect == false) {
				for (var h = 0; h < d; h++) {
					...
					if (h == 0 && r.caption == "" && r.captionHtml == ""
							&& d <= 1) {
						for (var g = 0; g < r.items.length; g++) {
							var q = r.items[g].value;
							if (r.items[g].value == undefined
									|| r.items[g].value == null) {
								q = g
							}
		...
		_mapItem : function(c) {
			...
			if (b.label == undefined && b.value == undefined
					&& b.html == undefined) {
				b.label = b.value = c
			}
			b.html = c.html || "";
		...
		insertAt : function(l, f) {
			...
			d.setPosition(0);
			if ((this.allowDrag && this._enableDragDrop)
					|| (this.virtualSize && this.virtualSize.height < 10
							+ this.host.height())) {
				this._addItems(true)
			} else {
