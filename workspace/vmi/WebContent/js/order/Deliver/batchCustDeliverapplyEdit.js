...
//			id:'DJ.order.Deliver.batchDeliverapplyEdit.faddress',
		...
		listeners:{
			select:function(combo,record){
				Ext.getCmp('DJ.order.Deliver.Deliverapplys').chooseRecord.faddressComb = record[0];
			}
			
		}
};

Ext.define('DJ.order.Deliver.batchCustDeliverapplyEdit',{
	extend : 'Ext.c.BaseEditUI',
	id:'DJ.order.Deliver.batchCustDeliverapplyEdit',
	title : "批量新增编辑界面",
	width : 1120,
	minHeight : 350,
	closable : true,
	...
	myobj: {},
	layout:'fit',
	url:'SavebatchDeliverapply.do',
	requires:'Ext.ux.form.DateTime2Field',
	onload:function(){
...
//					me.myobj.clearAddress = 1;
				...
				for(i=0;i<5;i++){
					arr.push({
						farrivetime:Ext.Date.add(new Date(new Date().setHours(14,0,0,0)),Ext.Date.DAY,2),
						famount:0,
	...
	Action_BeforeSubmit:function(){
		...
		if(Ext.getCmp('DJ.order.Deliver.batchDeliverapplyEdit.cutpdtnameComb')!=undefined){
			Ext.getCmp('DJ.order.Deliver.batchDeliverapplyEdit.cutpdtnameComb').inputEl.dom.value='0';
		}
		if(Ext.getCmp('DJ.order.Deliver.batchDeliverapplyEdit.faddressComb')!=undefined){
			Ext.getCmp('DJ.order.Deliver.batchDeliverapplyEdit.faddressComb').inputEl.dom.value='0';
		}
	},
	custbar:['','-','',{
		xtype:'textfield',
	...
	}/*,{xtype:'tbspacer',width:20}*/,Ext.applyIf({
		xtype:'cCombobox',
		displayField:'fname',
		valueField:'fid',
		fieldLabel:'配送地址',
...
//				}
					...
					success : function(response, option) {
						...
						if(obj.success){
							...
							if(obj.data &&　obj.data.length==1){
								...
								me.fireEvent('select', me,[{'data':record}]);
							}
						}
					}
				});
	...
	},addressConfig)/*,{xtype:'tbspacer',width:20}*/,{
			xtype : 'combobox',
	...
	initComponent:function(){
		Ext.apply(this,{
			items:[{
				...
				pageSize:100,
				url:'',
				id:'DJ.order.Deliver.Deliverapplys',
				...
				bbar : [Ext.create(
					...
					handler: function(){
						...
						win.close();
						relatedPanel.switchOrderType('single');
					}
				}],
...
//								e.grid.chooseRecord.cutpdtnameComb = '';
								Ext.getCmp('DJ.order.Deliver.batchDeliverapplyEdit.cutpdtnameComb').inputEl.dom.value='';
								
//								if(Ext.getCmp('DJ.order.Deliver.batchCustDeliverapplyEdit').myobj.clearAddress=="1"){
//									var rows = Ext.getCmp("DJ.order.Deliver.Deliverapplys").getSelectionModel().getSelection();
...
//								e.grid.chooseRecord.faddressComb = '';
								Ext.getCmp('DJ.order.Deliver.batchDeliverapplyEdit.faddressComb').inputEl.dom.value='';
...
//									return "<span data-qtip='包装物名称："+val+", 包装物编码："+record.get('fnumber')+", 规格："+record.get('fspec')+", 特性："+record.get('fcharactername')+"'>"+val+"</span>";
							...
							editor:{
								...
								displayField:'fname',
								valueField:'fid',
								id:'DJ.order.Deliver.batchDeliverapplyEdit.cutpdtnameComb',
								allowBlank:false,
								blankText:'请选择包装物',
//								multiSelect:true,
								MyConfig:{
									...
									height:200,
									hiddenToolbar:true,
									id:'DJ.order.Deliver.batchDeliverapplyEdit.CustProductList',
									url : 'GetCustproductLists.do',
									ShowImg: true,
									fields:[{
										name:'fid'
...
//										flex:1
								...
								listeners:{
									select:function(combo,record){
										Ext.getCmp('DJ.order.Deliver.Deliverapplys').chooseRecord.cutpdtnameComb = record[0];
										...
										/*Ext.Ajax.request({
											...
											success : function(response, option) {
												...
												if (obj.success == true) {
													...
													Ext.getCmp('DJ.order.Deliver.Deliverapplys').chooseRecord.balance = obj.data[0].balanceqty;
...
//									var me = this;
								...
								editor:Ext.apply({
									...
									displayField:'fname',
									valueField:'fid',
									id:'DJ.order.Deliver.batchDeliverapplyEdit.faddressComb',
									allowBlank:false,
									blankText:'请选择送货地址'
								...
								editor:{
									...
									listeners:{
										select:function(combo,record){
											Ext.getCmp('DJ.order.Deliver.Deliverapplys').chooseRecord.fsupplierRecord = record[0];
										},
