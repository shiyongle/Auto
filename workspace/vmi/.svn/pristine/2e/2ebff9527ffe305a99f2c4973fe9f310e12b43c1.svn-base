...
Ext.define('Ext.ux.DateTimePicker', {
	  ...
	  onRender: function(container, position) {
		  ...
		  var Browser=new Object();
			...
			if(Browser.isIE){ 
			table = Ext.get(Ext.dom.Query.select('table', this.getEl().dom)[1]); 
			}else{
			table = Ext.get(Ext.DomQuery.selectNode('table', this.el.dom));
			}
...
//		  return this.update(this.value);
		  return this.update(Ext.Date.clearTime(value, true));
	  },
	  // overwrite
	  getValue: function() {
	...
	showPrevYear : function() {
		return this.setValue(Ext.Date.add(this.value, Ext.Date.YEAR, -1))
	},
