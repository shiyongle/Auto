...
Ext.define('DJ.order.Deliver.OrderAppraiseSupplierList', {
	extend : 'Ext.c.GridPanel',
	alias : 'widget.orderappraisesupplierlist',	
	title : "订单评价明细表",
	id : 'DJ.order.Deliver.OrderAppraiseSupplierList',
	pageSize : 50,
	closable : false,// 是否现实关闭按钮,默认为false
	url : 'GetorderParisesSupplierList.do',
	Delurl : "",
	 EditUI : "DJ.order.Deliver.OrderappraiseEdit",
	exporturl : "OrderParisesSupplierListtoexcel.do",// 导出为EXCEL方法
	onload : function() {
		var me = this;	
		// //同时也刷新图表
		Ext.getCmp("DJ.order.Deliver.OrderAppraiseSupplierList.timeboxbeginTime").setValue(new Date(new Date().setDate(1)));
		Ext.getCmp("DJ.order.Deliver.OrderAppraiseSupplierList.timeboxendTime").setValue(new Date());
		me.getStore().on('load', function() {	
			...
			MyCommonToolsZ.setComAfterrender(me, function(com) {
					...
					var fieldT = com.down("textfield[id=DJ.order.Deliver.OrderAppraiseSupplierList.findBy]");

					MyCommonToolsZ.setQuickTip(fieldT.id, "", "可输入：客户名称、配送单号");

				});
		    Ext.getCmp('DJ.order.Deliver.OrderAppraiseSupplierList.querybutton').hide();
			Ext.getCmp('DJ.order.Deliver.OrderAppraiseSupplierList.addbutton').hide();
			Ext.getCmp('DJ.order.Deliver.OrderAppraiseSupplierList.editbutton').hide();
			Ext.getCmp('DJ.order.Deliver.OrderAppraiseSupplierList.delbutton').hide();
		//	Ext.getCmp('DJ.order.Deliver.OrderAppraiseList.viewbutton').hide();
//			Ext.getCmp('DJ.order.Deliver.OrderAppraiseList.exportbutton').hide();
	},
...
//		condictionFields:['c.fname','d.fnumber','o.fappraisetime'],//时间只能配置为最后
//		condictionFieldsDateFields:["o.fappraisetime"],
//		tip:'可输入:客户名称、配送单号、评价时间',
//		useDefaultfilter : true
	xtype : 'textfield',
		itemId : 'textfield',
		width:120,
		id : 'DJ.order.Deliver.OrderAppraiseSupplierList.findBy',
		emptyText : '回车搜索',
		...
		listeners:{
			...
			id:"DJ.order.Deliver.OrderAppraiseSupplierList.timebox",
			conditionDateField : "o.fappraisetime",
			labelFtext : "评价时间",
			useDefaultfilter : true,
