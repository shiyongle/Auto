...
public class UserCustomerController extends BaseController{	
	...
	public String exportExecl(HttpServletRequest request,HttpServletResponse reponse,@ModelAttribute UserCustomerQuery ucquery,Integer[] ids) throws Exception{
		...
		try {
            ...
            String headerArr[] = {"序号","客户编码","客户名称","客户简称","登录帐号","客户类型","客户所在区域","业务员","业务员所在部门","结算周期","认证时间"};
    		...
    		for(int i=0;i<list.size();i++){
    			...
    			wsheet.addCell(new Label( 5 , conut ,list.get(i).getFtype(),wcf2));
    			wsheet.addCell(new Label( 6 , conut ,list.get(i).getFregion() ,wcf2));
    			wsheet.addCell(new Label( 7 , conut ,list.get(i).getFsalesMan() ,wcf2));
    			wsheet.addCell(new Label( 8 , conut ,list.get(i).getFsalesManDept() ,wcf2));
    			wsheet.addCell(new Label( 9 , conut ,list.get(i).getFsettleCycle() ,wcf2));
    			wsheet.addCell(new Label( 10 , conut ,list.get(i).getCreateTimeString() ,wcf2));
    			conut++;
    		}
			wwb.write();
			os.flush();
