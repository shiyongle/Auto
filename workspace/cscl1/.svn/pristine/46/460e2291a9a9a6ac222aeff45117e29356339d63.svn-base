...
//					}
			...
			if(statementlist!=null)
			{
				for (CL_FinanceStatement statement : statementlist) {
					if(statement.getFbusinessType()==3&&statement.getFtype()==1)//异常费用
					{
		...
		if(user!=null&&!user.equals("null"))
		{
			...
			if(statementList!=null)  {
				for(CL_FinanceStatement find:statementList){
					if((find.getFbusinessType()==1||find.getFbusinessType()==2)&&find.getFtype()==-1){
						fcostFreight=fcostFreight.add(find.getFamount());/**运费支出*/
					}
					if(find.getFbusinessType()==3&&find.getFtype()==1)//异常费用 收入
					{
						freceive=freceive.add(find.getFamount());
					}
					if(find.getFbusinessType()==3&&find.getFtype()==-1)//异常费用 支出
					{
						fpay=fpay.add(find.getFamount());
					}
					if(find.getFbusinessType()==6)//充值收入
					{
						fcbalance=fcbalance.add(find.getFamount());
					}
					if(find.getFbusinessType()==7)//充值送收入
					{
						fgift=fgift.add(find.getFamount());
					}
				}
			}
			/**用户使用过的好运卷*/
		...
		try {
			...
			if(user!=null&&!user.equals("null"))
			{
				if(user.getVmiUserPhone()!=null&&!user.getVmiUserPhone().isEmpty())
				{
					...
					statementQuery.setIds(ids);
					statementQuery.setSortColumns("bt.fcreate_time desc");
					list = statementDao.find(statementQuery);
				}else{
					result.put("success", false);
					result.put("msg", "请先选择司机帐号!");
					return writeAjaxResponse(reponse,JSONUtil.getJson(result));
