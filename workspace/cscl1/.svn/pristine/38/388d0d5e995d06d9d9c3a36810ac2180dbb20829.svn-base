...
$(document).ready(function() {
	...
	$('#CLIdentificationTable').datagrid({
		...
		toolbar : [{
					...
					handler : function() {
						...
						if(row!=null && row.status==1){
							if(row.roleId==2){//针对车主显示
								$("#createWindow").window({
									left:"250px",
									top:"0px",
									title : "审核",
									href : '${ctx}/iden/vetted.do',
				...
				iconCls : 'icon-tip',
				handler : function() {
					var row=$("#CLIdentificationTable").datagrid('getSelections');
					if(row[0]!=null && row[0].status==3){
						if(row[0].roleId==2){//针对车主显示
							$.ajax({
					 			type : "POST",
								url : '${ctx}/iden/updateLuckDr.do?'+toStrId(row),
						...
						else if(row[0].roleId==1){
							$.messager.alert('提示','请选择待车主已通过的记录！');
						}
					}else if(row==null){
						$.messager.alert('提示','请选择一条记录！'); 
					}else{
				...
				iconCls : 'icon-edit',
				handler:function(){
					var row = $("#CLIdentificationTable").datagrid('getSelected');//获取选中的行
					if(row!=null && row.status==3&&row.roleName=='车主'){//选择一条已通过的数据进行修改，但是不能修改状态,并且需要对象是车主
						$("#createWindow").window({
								title : "修改",
								href : '${ctx}/iden/updateInfo.do?id=' + row.id,
								width : 710,
								height : 535,
								draggable:true,
								resizable:false,
						...
						$.messager.alert('提示','请选择已通过记录！');
					}else if(row.roleName!='车主'){
						$.messager.alert('提示','请选择车主记录！');
					}else if(row==null){
						$.messager.alert('提示','请选择一条记录！'); 
					}
				}//end handler
		...
		columns : [[
		            ...
		            {title : '车主名称',field : 'name',rowspan : 1,align : 'center',width : '250'},
...
// 		            {title : '认证类型',field : 'type',rowspan : 1,align : 'center',width : '150',formatter:formartType},
		            ...
		            {title : '驾驶员',field : 'driverName',rowspan : 1,align : 'center',width : '120'},
		            {title : '驾驶证号',field : 'driverCode',rowspan : 1,align : 'center',width : '120'},
		            {title : '车牌号',field : 'carNumber',rowspan : 1,align : 'center',width : '120'},
		            {title : '牌照类型',field : 'licenseType',rowspan : 1,align : 'center',width : '120',formatter:licenseType},
		            {title : '规格名称',field : 'specName',rowspan : 1,align : 'center',width : '100'},
		            {title : '车型名称',field : 'typeName',rowspan : 1,align : 'center',width : '120'},
		            {title : '好运司机',field : 'fluckDriver',rowspan : 1,align : 'center',width : '100',formatter:formartfluckDriver},
		            {title : '注册时间',field : 'registerTime',rowspan : 1,align : 'center',width : '120',formatter:localTime},
		            {title : '主要活动区域',field : 'activeArea',rowspan : 1,align : 'center',width : '120',formatter:area}
		           ]],
...
function licenseType(value,rowData,rowIndex){
	if(value==0){
		return '<span>黄牌</span>';
	}else if(value==1){
...
function formartfluckDriver(value,rowData,rowIndex){
	if(value==0){
		return '<span style="color:green;font-weight:bold;">普通司机</span>';
	}else if(value==1){
		return '<span style="color:red;font-weight:bold;">好运司机</span>';
	}else {
		return '<span style="color:blue;font-weight:bold;">无</span>';
	}
}
//datagird列的格式化
			...
			<form id="searchIdenForm" method="post">
				...
				<table style="margin: 0 auto; margin-top: 0px;">
					<tr>
						...
						<td>
							<input type="hidden" value="car" name="roletype"/>
...
<script type="text/javascript">
	...
	sendMessage();
	setInterval("sendMessage()",60000);//一分钟查询一次
</script>
</body>
</html>