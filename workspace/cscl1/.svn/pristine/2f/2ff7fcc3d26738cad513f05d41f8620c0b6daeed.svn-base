...
<html lang="zh-CN">
  ...
  <meta charset="utf-8">
    ...
    <script src="${ctx}/pages/pcWeb/js/index_customer.js"></script>
	...
	<div class="container index_container">
		<div class="row">
			<div class="col-xs-12 text-center pay">
				收入: <span class="redClolr" id="income">2747.50</span> 元　　　支出: <span class="green" id="expenditure">2747.50</span> 元
			</div>
		</div>
...
$(function(){
	...
	$.ajax({
		type:'post',
		url:'${ctx}/pcWeb/pcWebPay/paymentSituation.do?type=1',
		dataType:'json',
		success:function(response){
	...
	$.ajax({
		...
		dataType:'json',
		success:function(response){
			$('#expenditure').text(response.data);
		}
	})
	//折线图
	$.ajax({
...
//		 			return Math.round(Math.random() * 1000 * (i + 1));
	...
	$.ajax({
		type:"post",
		url:"${ctx}/pcWeb/select/findOrderByType.do?type=1",
		dataType:"json",
		success:function(response){			
