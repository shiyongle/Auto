...
<script type="text/javascript" src="${ctx}/pages/pcWeb/js/my_order.js"></script>
</head>
<body>
	...
	<div class="jumbotron main">
		...
		<div class="row">
			<div class="col-sm-12 time_recent" id="state_type">
				...
				<a href="javascript:void(0)" class="" data-type='0'>全部</a>
				<a href="javascript:void(0)" data-type='1'>待付款</a>
				<a href="javascript:void(0)" data-type='2'>运输中</a>
				<a href="javascript:void(0)" data-type='3'>待评价</a>
				</div>
			</div>
		</div>
...
<!-- 		</div> -->
		...
		<table class="table  table-hover" id="my_order_table"></table>
		<!--页码-->
		<div class="row">					
...
var size=15;//一页显示多少条
var total;//总共多少条记录
var url="${ctx}/pcWeb/order/pcWebcycleorder.do?status=0&pagesize="+size+"&pagenum=1";
...
//	最近天数选择,搜索
	$("#recent_time a").click(function(){
		...
		var newurl="${ctx}/pcWeb/order/pcWebcycleorder.do?status=0&pagesize="+size+"&pagenum=1"+"&key="+search_key;
		pageOrderSearch(total,size,newurl);
...
// 	layer.open({
// 		  title: '追加款',
//   		  content: '<input type="number" class="form-control" id="addpay" name="addpay"/>'+
//   		  			'<div style="color:#999;">(请输入追加款金额)</div>',
// 		  yes: function(index,layero){
// 			  var moneyAdd=$("#addpay").val();//得到追加款金额	
// 			  $.ajax({
// 			  	type:"post",
// 			  	url:"",//追加款接口	
// 			  	data:moneyAdd,
...
// 			  	if(response.success=="true"){
// 			  		layer.msg("追加款成功！");			  		
// 			  		pageOrderSearch(total,size,url);//刷新表格数据	
// 			  	}
// 			  	}
// 			  });
// 		   	  layer.close(index);		    
...
// 	});    
	...
	//location.href="${ctx}/pages/pcWeb/payment/paymentAdd.jsp?orderId="+id;
 	event.stopPropagation();//阻止冒泡,避免tr的点击事件
